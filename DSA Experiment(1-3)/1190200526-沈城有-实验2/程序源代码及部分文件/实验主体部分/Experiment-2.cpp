...
#define CompDir "Comp.ht"         //压缩文本保存路径（写入+读取）
#define DeCompDir "Decomp.txt"    //解压文本保存路径（写入）
...
int CompressFile(void);                                        //根据编码生成压缩文件
void CalculateRate(int counts, int countt);                    //计算压缩率
...
int ReadFromFile(void)
{
	...
	map<char, int> data;         //统计字符及频率
	map<char, int>::iterator it; //查找字符所用迭代器
	char temp;
	int countt = 0;
	double rate;
	if (!OpenFile)
...
int CompressFile(void)
{
	...
	for (unsigned int k = result.size(); k < 2 * result.size() - 1; ++k)  //写入哈夫曼树中lchild、rchild（解码无需回溯故略去parent)
		WriteFile << (char)HTree[k].lchild << (char)HTree[k].rchild;
...
void CalculateRate(int counts, int countt)
{
	...
	cout << "Compress rate:" << crate << "%" << endl;           //真实文件压缩率
	cout << "Encode rate:" << srate << "%" << endl;             //编码率
}
...
bool DecompressFile(void)
{
	...
	for (int k = count; k <= end; ++k)
	{
		ReadFile >> b1 >> b2;
		HTree[k].lchild = (int)b1;
		HTree[k].rchild = (int)b2;
	}
	ReadFile >> b1 >> b2 >> b3;
	do{
		if (!((b1 >> (7 - i)) & 1))
